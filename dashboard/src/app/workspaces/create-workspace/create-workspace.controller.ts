...
export class CreateWorkspaceController {
  ...
  /**
   * The workspace config of current stack.
   */
  ...
  /**
   * The runtime environment of selected stack.
   */
  ...
  onStackSelected(stackId: string): void {
    ...
    this.workspaceConfig = this.stack.workspaceConfig;
    this.environmentName = this.workspaceConfig.defaultEnv;
    this.environment = this.workspaceConfig.environments[this.environmentName];
    const recipeType = this.environment.recipe.type;
    this.environmentManager = this.cheEnvironmentRegistry.getEnvironmentManager(recipeType);

    this.stackMachines = this.environmentManager.getMachines(this.environment);
  }
  ...
  onRamChanged(name: string, memoryLimitBytes: number): void {
    const machine = this.stackMachines.find((machine: IEnvironmentManagerMachine) => {
      return machine.name === name;
    });

    this.environmentManager.setMemoryLimit(machine, memoryLimitBytes);

    this.environment = this.environmentManager.getEnvironment(this.environment, this.stackMachines);
  }
  ...
  createWorkspace(): void {
    console.log('>>> this.namespaceId: ', this.namespaceSelectorSvc.getNamespaceId());
